...
export function buildDashboardMetrics(
  ...
  const totalNet = approvedValuations.reduce(
...
export function exportLiquidationPdf(
  ...
  doc.text(
    `Deducciones: ${formatCurrency(Number(liquidation.total_deductions_amount), project.currency_code)}`,
    14,
    44,
  );
  doc.text(
    `Monto final: ${formatCurrency(Number(liquidation.final_amount), project.currency_code)}`,
    14,
    52,
  );
