...
export async function generateMetradosPdf(args: GenerateMetradosPdfArgs) {
  ...
  const ResumenHeader = h(
    ...
    h(
      Text,
      { style: [styles.th, { width: resumenWidths.partida, textAlign: "center" }] as any },
      "PARTIDA",
    ),
    h(
      Text,
      { style: [styles.th, { width: resumenWidths.desc, textAlign: "center" }] as any },
      "DESCRIPCIÓN",
    ),
    h(
      Text,
      { style: [styles.th, { width: resumenWidths.und, textAlign: "center" }] as any },
      "UND.",
    ),
    h(
      Text,
      {
        style: [
          styles.th,
          { width: resumenWidths.total, textAlign: "center", borderRightWidth: 0 },
        ] as any,
      },
      "TOTAL",
    ),
  ...
  const resumenBody = hierarchy.map((row, idx) => {
      ...
      return h(
        ...
        h(
          Text,
          {
            style: [
              styles.td,
              { width: resumenWidths.partida, fontFamily: "Helvetica-Bold" },
            ] as any,
          },
          row.code,
        ),
        ...
        h(
          Text,
          { style: [styles.td, { width: resumenWidths.total, borderRightWidth: 0 }] as any },
          "",
        ),
      );
    }
    return h(
      ...
      h(
        Text,
        { style: [styles.td, { width: resumenWidths.und, textAlign: "center" }] as any },
        row.unit || "",
      ),
      h(
        Text,
        {
          style: [
            styles.td,
            { width: resumenWidths.total, textAlign: "right", borderRightWidth: 0 },
          ] as any,
        },
  ...
  const ResumenPage = h(
    ...
    hierarchy.length === 0
      ? h(
          Text,
          { style: styles.p } as any,
          "Sin metrados registrados para el período seleccionado.",
        )
  ...
  const PlanillaHeader = h(
    ...
    h(
      ...
      h(
        Text,
        {
          style: [
            styles.th,
            { width: planWidths.total, textAlign: "center", borderRightWidth: 0 },
          ] as any,
        },
        "TOTAL",
      ),
    ...
    h(
      ...
      h(
        Text,
        {
          style: [styles.th, { width: planWidths.partida, borderRightColor: COLORS.border }] as any,
        },
        "",
      ),
      ...
      h(
        Text,
        { style: [styles.th, { width: planWidths.largo, textAlign: "center" }] as any },
        "LARGO",
      ),
      h(
        Text,
        { style: [styles.th, { width: planWidths.ancho, textAlign: "center" }] as any },
        "ANCHO",
      ),
      h(
        Text,
        { style: [styles.th, { width: planWidths.alto, textAlign: "center" }] as any },
        "ALT.",
      ),
      h(
        Text,
  ...
  for (let i = 0; i < executedItems.length; i++) {
      ...
      h(
        ...
        h(
          Text,
          {
            style: [
              styles.td,
              { width: planWidths.und, textAlign: "center", fontFamily: "Helvetica-Bold" },
            ] as any,
          },
          item.unit || "",
        ),
      ...
      planillaRows.push(
        h(
          ...
          h(
            ...
            line.num_elements != null && Number(line.num_elements) !== 0
              ? String(line.num_elements)
              : "",
          ),
          h(
            Text,
    ...
    planillaRows.push(
      h(
        ...
        h(
          Text,
          { style: [styles.td, { width: planTotalWidth, borderRightWidth: 0, height: 6 }] as any },
          "",
        ),
      ),
    );
  ...
  const planillaTitleBlock = h(
    ...
    h(
      ...
      h(
        Text,
        { style: { fontFamily: "Helvetica-Bold", fontSize: 10, textAlign: "center" } } as any,
    ...
    h(
      ...
      h(
        Text,
        { style: { width: 70, fontFamily: "Helvetica-Bold", fontSize: 8 } } as any,
        "PROYECTO:",
      ),
    ...
    h(
      ...
      h(
        ...
        clean((project as any).client_name ?? (project as any).client ?? "—"),
      ),
    ),
    h(
      View,
  ...
  const PlanillasPage = h(
    ...
    executedItems.length === 0
      ? h(
          Text,
          { style: [styles.p, { marginTop: 8 }] } as any,
          "Sin partidas con líneas registradas en el período.",
        )
