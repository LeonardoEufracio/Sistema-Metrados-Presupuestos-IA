...
export async function generateExpedienteClientPdf(args: GenerateArgs) {
  ...
  function Table(props: {
    ...
    return h(
      View,
      { style: styles.table, wrap: true } as any,
      // header
      h(
        View,
      ...
      ...rows.map((r, ri) =>
        h(
          ...
          {
            key: `r-${ri}`,
            style: ri === rows.length - 1 && !totalRow ? styles.trLast : styles.tr,
            wrap: false,
          } as any,
          ...cols.map((c, i) =>
            h(
  ...
  const Ficha = h(
    ...
    FichaRow({
      label: "Costo directo",
      value: formatMoney(Number(project.direct_cost || 0), currency),
    }),
    FichaRow({
      label: "Gastos generales",
      value: formatMoney(Number(project.overhead_cost || 0), currency),
    }),
    FichaRow({
      label: "Utilidad",
      value: formatMoney(Number(project.utility_amount || 0), currency),
    }),
  ...
  const ResumenFinalPage = h(
    ...
    h(
      View,
      {
        style: { marginTop: 8, borderTopWidth: 1, borderTopColor: COLORS.accent, paddingTop: 6 },
      } as any,
      h(
        View,
        { style: { flexDirection: "row", justifyContent: "space-between" } } as any,
        h(
          Text,
          { style: { fontFamily: "Helvetica-Bold", fontSize: 11 } } as any,
          "MONTO NETO A PAGAR",
        ),
        h(
          Text,
