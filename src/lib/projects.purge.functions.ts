...
export const purgeProject = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [budgetFiles, projectDocs, memoriaDocs, valuationDocs, liquidationDocs] =
      await Promise.all([
    ...
    if (budgetFiles.error)
      throw new Error(`No se pudieron leer archivos de presupuesto: ${budgetFiles.error.message}`);
    if (projectDocs.error)
      throw new Error(
        `No se pudieron leer documentos del expediente: ${projectDocs.error.message}`,
      );
    if (memoriaDocs.error)
      throw new Error(`No se pudieron leer PDFs de memoria: ${memoriaDocs.error.message}`);
    if (valuationDocs.error)
      throw new Error(`No se pudieron leer PDFs de valorización: ${valuationDocs.error.message}`);
    if (liquidationDocs.error)
      throw new Error(`No se pudieron leer PDFs de liquidación: ${liquidationDocs.error.message}`);

    budgetFiles.data?.forEach((row) =>
      addStoragePath(storagePaths, "budget-imports", row.file_path),
    );
    ...
    for (const bucket of buckets) {
      ...
      if (listError)
        throw new Error(`No se pudo listar archivos en ${bucket}: ${listError.message}`);
      list?.forEach((file) =>
        addStoragePath(storagePaths, bucket, `${data.projectId}/${file.name}`),
      );
    ...
    if ((projectCount ?? 0) !== 1) {
      throw new Error(
        "La operación terminó sin eliminar el proyecto padre. No se confirmó la purga.",
      );
    }
