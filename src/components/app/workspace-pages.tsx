...
function compareBudgetItemCodes(
  ...
  for (let index = 0; index < length; index += 1) {
    ...
    if (partA !== partB)
      return partA.localeCompare(partB, "es", { numeric: true, sensitivity: "base" });
  }
...
function getBudgetHierarchyStats<T extends BudgetHierarchyItem>(items: T[]) {
  const parentSet = buildParentCodeSet(
    items.map((item) => ({ item_code: item.item_code ?? null })),
  );
...
function renderBudgetDescription(item: BudgetHierarchyItem) {
  const level =
    ...
    Math.max((item.item_code ?? "").split(".").filter(Boolean).length - 1, 0);
  return (
          ...
          <section className="space-y-8">
            <div className="space-y-5">
              ...
              <p className="max-w-xl text-base leading-relaxed text-muted-foreground">
                ...
                <span className="font-medium text-foreground">JJ&amp;PP Ingenieros</span> para
                registrar metrados ejecutados, controlar memorias valorizadas, calcular
                valorizaciones mensuales y consolidar la liquidación final con trazabilidad
                completa.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div
                  key={title}
                  className="rounded-lg border border-border/60 bg-card/50 p-4 backdrop-blur-sm"
                >
...
function DeleteOrArchiveProjectDialog({
  ...
  const totalMovement = movement ? Object.values(movement).reduce((sum, n) => sum + n, 0) : 0;
  const isEmpty = movement !== null && totalMovement === 0;

  const handleDelete = async () => {
    if (
      !confirm(
        `¿Eliminar permanentemente el proyecto "${project.name}"? Esta acción no se puede deshacer.`,
      )
    )
      return;
    setBusy(true);
  ...
  const handleStatusChange = async (newStatus: "archived" | "closed") => {
    const verb = newStatus === "archived" ? "archivar" : "cancelar";
    if (!confirm(`¿Confirmas ${verb} el proyecto "${project.name}"? Se conservará el historial.`))
      return;
    setBusy(true);
    setError(null);
    ...
    toast.success(newStatus === "archived" ? "Proyecto archivado." : "Proyecto cerrado (cancelado).");
    setOpen(false);
    await onDone();
  };
  ...
  const handlePurge = async () => {
    ...
    try {
      if (!session?.access_token) {
        throw new Error(
          "No hay una sesión válida para ejecutar la purga. Vuelve a iniciar sesión e inténtalo nuevamente.",
        );
      }
      ...
      if (verifyError)
        throw new Error(
          `La purga terminó, pero no se pudo refrescar/verificar la lista: ${verifyError.message}`,
        );
      if (stillExists)
        throw new Error("La purga terminó, pero el proyecto todavía aparece en la base de datos.");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {purgeMode ? "Purgar proyecto" : "Eliminar o archivar proyecto"}
          </DialogTitle>
          <DialogDescription>
          ...
          <div className="space-y-4">
            {purgeMode ? (
              <div className="space-y-3">
                <div className="rounded-md border border-destructive/60 bg-destructive/10 p-3 text-sm">
                  ...
                  <p className="mt-1">
                    ...
                    <span className="font-medium">
                      el proyecto y TODOS sus registros relacionados
                    </span>
                    : ficha técnica, presupuesto, partidas, metrados, memoria valorizada,
                    valorizaciones, deducciones, documentos y archivos. El sistema quedará como si
                    el proyecto nunca hubiera existido.
                  </p>
                </div>
                {totalMovement > 0 ? (
                  <ul className="ml-4 list-disc space-y-1 text-xs text-muted-foreground">
                    {(Object.keys(movement) as Array<keyof ProjectMovementCheck>)
                      ...
                      .map((k) => (
                        <li key={k}>
                          {movementLabels[k]}:{" "}
                          <span className="font-medium text-foreground">{movement[k]}</span>
                        </li>
                      ))}
                  </ul>
                ) : null}
                <label className="flex items-start gap-2 text-sm">
                  ...
                  <span>
                    ...
                    <span className="font-medium">permanente e irreversible</span>.
                  </span>
                </label>
                <div>
              ...
              <div className="rounded-md border border-border bg-muted/30 p-3 text-sm">
                Este proyecto{" "}
                <span className="font-medium text-foreground">no tiene información asociada</span>.
                Puedes eliminarlo permanentemente.
              </div>
            ) : (
              <div className="space-y-2">
                <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 text-sm">
                  Este proyecto tiene información registrada y{" "}
                  <span className="font-medium">no puede eliminarse físicamente</span> con la acción
                  estándar. Puedes archivarlo, cancelarlo o{" "}
                  {isAdmin ? "purgarlo (admin)" : "solicitar a un administrador la purga total"}.
                </div>
                <ul className="ml-4 list-disc space-y-1 text-xs text-muted-foreground">
                  {(Object.keys(movement) as Array<keyof ProjectMovementCheck>)
                    .filter((k) => movement[k] > 0)
        ...
        <Form {...form}>
          <form className="space-y-4" onSubmit={submit}>
            <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        {...field}
                        value={field.value ?? ""}
                        placeholder="Municipalidad / Entidad contratante"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
              ...
              <FormField
                control={form.control}
                name="execution_modality"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Modalidad de ejecución *</FormLabel>
                    <FormControl>
                      <Input
                        {...field}
                        value={field.value ?? ""}
                        placeholder="Contrata / Administración directa"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        {...field}
                        value={field.value ?? ""}
                        placeholder="Distrito, provincia, región"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Contrato de ejecución *</FormLabel>
                    <FormControl>
                      <Input {...field} value={field.value ?? ""} placeholder="N° de contrato" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                control={form.control}
                name="supervision_contract"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Contrato de supervisión *</FormLabel>
                    <FormControl>
                      <Input {...field} value={field.value ?? ""} placeholder="N° de contrato" />
                    </FormControl>
                    <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Monto contractual *</FormLabel>
                    <FormControl>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        value={field.value ?? 0}
                        onChange={(e) => {
                          lastEditedRef.current = "term";
                          field.onChange(e);
                        }}
                      />
                    </FormControl>
                    <FormDescription>
                      Se calcula automáticamente desde las fechas de inicio y término (días
                      calendario, inclusivos).
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="P-2026-001" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Obligatorio. No podrá modificarse luego del inicio.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              return [
                ...
                <div key={`a-${project.id}`} className="flex flex-wrap gap-2">
                  ...
                  <DeleteOrArchiveProjectDialog
                    project={project}
                    onDone={(result) =>
                      result?.removed ? refreshProjectsAfterRemoval(project.id) : refresh()
                    }
                  />
                </div>,
              ];
        ...
        <div className="flex flex-col gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Se detectan columnas como código, descripción, unidad, metrado y precio unitario.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {preview ? (
                <div className="rounded-md border border-border bg-muted/40 p-3 text-sm">
                  <p className="font-medium text-foreground">
                    Se detectaron {preview.rows.length} partidas en el archivo.
                  </p>
                  {preview.warnings.length ? (
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Revisa la vista previa completa antes de confirmar la importación.
                    </p>
                  )}
                </div>
              ) : null}
              {message ? <p className="text-sm text-primary">{message}</p> : null}
              <Button onClick={() => void uploadBudget()} disabled={!preview || !selectedProjectId}>
                ...
                return (
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Revisa todas las partidas detectadas y el subtotal general antes de
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid gap-3 sm:grid-cols-2">
                        <div className="rounded-md border border-border bg-muted/40 p-3">
                          <p className="text-xs uppercase tracking-wide text-muted-foreground">
                            Ítems hoja ejecutables
                          </p>
                          <p className="text-2xl font-semibold text-foreground">
                            {previewStats.leafCount}
                          </p>
                          <p className="mt-1 text-xs text-muted-foreground">
                            {previewStats.nodeCount} nodos jerárquicos importados
                          </p>
                        </div>
                        <div className="rounded-md border border-primary/30 bg-primary/5 p-3">
                          <p className="text-xs uppercase tracking-wide text-muted-foreground">
                            Subtotal general del presupuesto
                          </p>
                          <p className="text-2xl font-semibold text-primary">
                            {formatCurrency(grandTotal)}
                          </p>
                          <p className="mt-1 text-xs text-muted-foreground">
                            Calculado solo con nodos hoja
                          </p>
                        </div>
                      </div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {previewStats.nodeCount} nodos mostrados · {previewStats.leafCount} ítems
                        hoja · subtotal general sin doble conteo.
                      </p>
                    </CardContent>
                  </Card>
                );
            ...
            return (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Listado completo de partidas cargadas para el proyecto seleccionado. Use el
                    scroll interno para revisarlas todas.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-lg border border-border bg-muted/30 p-4">
                      ...
                      <p className="mt-1 text-2xl font-semibold text-foreground">
                        {registeredStats.leafCount} partidas finales
                      </p>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {registeredStats.nodeCount} nodos jerárquicos registrados
                      </p>
                    </div>
                    <div className="rounded-lg border border-border bg-muted/30 p-4">
                      <p className="text-xs uppercase tracking-wide text-muted-foreground">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        Suma exclusiva de ítems hoja
                      </p>
                    </div>
                  </div>
                  {currentItems.length === 0 ? (
                    <p className="text-sm text-muted-foreground">
                      No hay partidas registradas para este proyecto todavía.
                    </p>
                  ) : (
                    <ScrollableImportTable
                      headers={[
                      ...
                      rows={[
                        ...
                        [
                          "",
                          <span key="total-label" className="font-semibold">
                            Subtotal general
                          </span>,
                          <span key="total-type" className="font-semibold">
                            Solo hojas
                          </span>,
                          "",
                          "",
                          "",
                          <span key="total-value" className="font-semibold">
                            {formatCurrency(registeredTotal)}
                          </span>,
                        ],
                      ]}
  ...
  async function handleGeneratePdf() {
    ...
    try {
      ...
      const res = await generateMetradosPdf({
        ...
        items: projectItems,
        currentLines: lines as MetradoLine[],
      });
      toast.dismiss(tid);
      ...
      toast.error(e instanceof Error ? e.message : "Error generando el documento.");
    } finally {
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <Select
                ...
                disabled={!projectId || !periods.length}
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={periods.length ? "Selecciona período" : "Sin períodos creados"}
                  />
                </SelectTrigger>
                <SelectContent>
                  {periods.map((p) => (
              ...
              {!periods.length && projectId ? (
                <p className="mt-2 text-xs text-muted-foreground">
                  Crea un período desde Memoria valorizada e Informe Técnico antes de capturar
                  metrados.
                </p>
              ) : null}
            </div>
              ...
              {!activeItem ? (
                <Card>
                  <CardContent className="py-10 text-center text-sm text-muted-foreground">
                    Selecciona una subpartida ejecutable del panel izquierdo para registrar líneas
                    de metrado.
                  </CardContent>
                </Card>
              ) : (
                  ...
                  <CardHeader className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                      ...
                      <span>
                        Metrado base:{" "}
                        <span className="font-medium text-foreground">
                          {formatNumber(Number(activeItem.base_quantity || 0), 4)}
                        </span>
                      </span>
                      <span>
                        Acumulado período:{" "}
                  ...
                  <CardContent>
                    <div className="mb-3 flex justify-end">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => void addLine(activeItem.id)}
                      >
                        ...
                        <TableBody>
                          ...
                          {activeItemLines.length === 0 ? (
                            <TableRow>
                              ...
                              >
                                Sin detalles de cálculo. Usa “+ Agregar línea de metrado” para
                                añadir el sustento (tramo, sector, etc.).
                              </TableCell>
                            </TableRow>
                          ) : (
            ...
            <CardContent className="space-y-4">
              <Form {...form}>
                <form className="space-y-4" onSubmit={submit}>
                  ...
                  />
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground">Contenido técnico</label>
                ...
                return (
                  <div key={memoria.id} className="rounded-lg border border-border p-4">
                    ...
                    <p className="mt-3 line-clamp-3 text-sm text-muted-foreground">
                      {rich.plainText || memoria.executive_summary || "Sin detalle"}
                    </p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => updateStatus(memoria.id, "in_review")}
                      >
                        Enviar a revisión
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => updateStatus(memoria.id, "approved")}
                      >
                        Aprobar
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => updateStatus(memoria.id, "rejected")}
                      >
                        Observar
                      </Button>
                      {project ? (
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => exportMemoriaPdf(project, memoria)}
                        >
                          PDF
                        </Button>
                      ) : null}
...
export function ValuationsPage() {
  ...
  const createValuation = form.handleSubmit(async (values) => {
    ...
    if (!project || !memoria) {
      form.setError("root", {
        message: "Debe existir una memoria valorizada para el período seleccionado.",
      });
      return;
    }

    const entries = metrados.filter(
      (entry) =>
    ...
    const lines = Object.entries(grouped).map(([itemId, quantity]) => {
      ...
      return {
        ...
        line_amount:
          project.contract_type === "suma_alzada"
    ...
    <AuthGuard>
      ...
      >
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <Card>
            <CardHeader>
              <CardTitle>Generar valorización</CardTitle>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                <form className="space-y-4" onSubmit={createValuation}>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>% avance (suma alzada)</FormLabel>
                          <FormControl>
                            <Input type="number" step="0.01" {...field} />
                          </FormControl>
                          <FormDescription>
                            Solo aplica para contratos a suma alzada.
                          </FormDescription>
                        </FormItem>
                      )}
                    />
                  ...
                  <div key={valuation.id} className="mb-4 rounded-lg border border-border p-4">
                    ...
                    <div className="mt-3 grid gap-2 text-sm text-muted-foreground md:grid-cols-3">
                      <p>
                        Bruto:{" "}
                        {formatCurrency(
                          Number(valuation.gross_amount),
                          project?.currency_code || "PEN",
                        )}
                      </p>
                      <p>
                        Deducciones:{" "}
                        {formatCurrency(
                          Number(valuation.deductions_amount),
                          project?.currency_code || "PEN",
                        )}
                      </p>
                      <p>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => updateStatus(valuation.id, "reviewed")}
                      >
                        Revisar
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => updateStatus(valuation.id, "approved")}
                      >
                        Aprobar
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => updateStatus(valuation.id, "rejected")}
                      >
                        Rechazar
                      </Button>
                      {project ? (
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => exportValuationPdf(project, valuation, lines)}
                        >
                          PDF
                        </Button>
                      ) : null}
...
export function ApprovalsPage() {
  ...
  return (
    <AuthGuard>
      <PageLayout
        title="Aprobaciones"
        description="Cola operativa para revisión del residente y aprobación del supervisor."
      >
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              <CardTitle>Memorias en revisión</CardTitle>
            </CardHeader>
            <CardContent>
              <SectionTable
                headers={["Proyecto", "Periodo", "Estado"]}
                rows={pendingMemorias.map((item) => [
                  projects.find((project) => project.id === item.project_id)?.name || "Proyecto",
                  getPeriodLabel(item.period_month),
          ...
          <Card>
            <CardHeader>
              <CardTitle>Valorizaciones por decidir</CardTitle>
            </CardHeader>
            <CardContent>
              <SectionTable
                headers={["Proyecto", "Periodo", "Neto", "Estado"]}
                rows={pendingValuations.map((item) => [
                  projects.find((project) => project.id === item.project_id)?.name || "Proyecto",
                  getPeriodLabel(item.period_month),
                  formatCurrency(Number(item.net_amount)),
                  valuationStatusLabels[item.status],
                ])}
              />
            </CardContent>
          </Card>
  ...
  return (
    <AuthGuard>
      <PageLayout
        ...
        actions={
          <Button variant="outline" onClick={() => exportFinancialWorkbook(projects, valuations)}>
            Exportar Excel
          </Button>
        }
      >
        <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
          <Card>
            ...
            <CardContent>
              <SectionTable
                headers={["Proyecto", "Contrato", "Valorizado aprobado"]}
                rows={projects.map((project) => [
                  ...
                  formatCurrency(
                    valuations
                      .filter(
                        (item) => item.project_id === project.id && item.status === "approved",
                      )
          ...
          <Card>
            <CardHeader>
              <CardTitle>Exportes rápidos</CardTitle>
            </CardHeader>
              ...
              {projects.map((project) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {budgetItems.filter((item) => item.project_id === project.id).length} partidas
                      · {metrados.filter((item) => item.project_id === project.id).length} metrados
                    </p>
                  </div>
                  <Button
                    size="sm"
                    ...
                    onClick={() =>
                      exportMetradosWorkbook(
                        project,
                        metrados.filter((item) => item.project_id === project.id),
                        Object.fromEntries(budgetItems.map((item) => [item.id, item.description])),
                      )
                    }
                  >
                    Excel metrados
              ...
              <Form {...form}>
                <form className="space-y-4" onSubmit={submit}>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Deducciones finales</FormLabel>
                        <FormControl>
                          <Input type="number" step="0.01" {...field} />
                        </FormControl>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Resumen de cierre</FormLabel>
                        <FormControl>
                          <Textarea {...field} value={field.value ?? ""} />
                        </FormControl>
                      </FormItem>
          ...
          <Card>
            ...
            <CardContent>
              {liquidations.map((liquidation) => {
                const project = projects.find((item) => item.id === liquidation.project_id);
                const relatedValuations = valuations.filter(
                  (item) =>
                    item.project_id === liquidation.project_id && item.status === "approved",
                );
                return (
                  <div key={liquidation.id} className="mb-4 rounded-lg border border-border p-4">
                    <p className="text-sm font-medium text-foreground">
                      {project?.name || "Proyecto"}
                    </p>
                    <p className="mt-2 text-sm text-muted-foreground">
                      Monto final:{" "}
                      {formatCurrency(
                        Number(liquidation.final_amount),
                        project?.currency_code || "PEN",
                      )}
                    </p>
                    <div className="mt-3 flex gap-2">
                      {project ? (
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            exportLiquidationPdf(project, liquidation, relatedValuations)
                          }
                        >
                          PDF
                        </Button>
                        ...
                        <Button
                          ...
                          onClick={() =>
                            exportValuationPdf(
                              project,
                              valuation,
                              valuationLines.filter((line) => line.valuation_id === valuation.id),
                            )
                          }
                        >
                          Valorización PDF
                        ...
                        <Button
                          ...
                          onClick={() =>
                            exportLiquidationPdf(
                              project,
                              liquidation,
                              valuations.filter((valuation) => valuation.project_id === project.id),
                            )
                          }
                        >
                          Liquidación PDF
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Button
                      ...
                      onClick={() =>
                        exportMetradosWorkbook(
                          project,
                          metrados.filter((entry) => entry.project_id === project.id),
                          itemMap,
                        )
                      }
                    >
  ...
  return (
    <AuthGuard requireAdmin>
      ...
      >
        {isAdmin ? (
          <SectionTable
            headers={["Usuario", "Cargo", "Roles", "Asignar"]}
            rows={profiles.map((profile) => [
              ...
              <div key={profile.id} className="flex gap-2">
                <Select
                  ...
                  onValueChange={(value) =>
                    setSelectedRole((current) => ({ ...current, [profile.user_id]: value }))
                  }
                >
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => void assignRole(profile.user_id)}
                >
                  Asignar
                </Button>
              </div>,
            ...
            <Form {...form}>
              <form className="space-y-4" onSubmit={submit}>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>URL de firma</FormLabel>
                      <FormControl>
                        <Input {...field} value={field.value ?? ""} />
                      </FormControl>
                      <FormDescription>
                        Usa una imagen alojada en un servicio seguro o bucket interno.
                      </FormDescription>
                    </FormItem>
                  )}
                />
          ...
          <Card key={title}>
            <CardHeader>
              ...
              <CardDescription>
                {
                  [
                    "Registro trazable de partidas, metrados y memorias valorizadas.",
                    "Cálculo por tipo contractual con revisión y aprobación.",
                    "Consolidación económica de obra con exportes listos.",
                  ][index]
                }
              </CardDescription>
            </CardHeader>
